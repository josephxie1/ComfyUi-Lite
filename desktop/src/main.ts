...
import { createTray } from './main-process/tray';
...
async function startApp() {
  ...
  // Create tray icon (macOS menu bar)
  createTray();
...
function initalizeLogging() {
  ...
  log.info(`Starting ComfyUI Lite v${app.getVersion()}`);
}

/** Quit when all windows are closed.*/
function quitWhenAllWindowsAreClosed() {
  app.on('window-all-closed', () => {
    log.info('Quitting ComfyUI Lite because window all closed');
    app.quit();
  });
}
