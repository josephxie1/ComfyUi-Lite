...
export class VirtualEnvironment implements HasTelemetry, PythonExecutor {
  ...
  constructor(
    ...
    function compiledRequirements() {
      ...
      if (process.platform === 'win32') return 'windows_cpu';
    }
  }
  ...
  private async runUvCommandAsync(args: string[], callbacks?: ProcessCallbacks): Promise<{ exitCode: number | null }> {
    log.info('Running uv command (spawn):', this.uvPath, args.join(' '));
    // ComfyUI Lite: Use child_process.spawn instead of node-pty to avoid posix_spawnp errors in unsigned macOS apps
    return this.runUvAsync(args, callbacks);
  }
  ...
  private async manualInstall(callbacks?: ProcessCallbacks): Promise<void> {
    // ComfyUI Lite: No PyTorch or Manager installation needed
   ...
   * @param callbacks The callbacks to use for the command.
   */
  // ComfyUI Lite: PyTorch installation methods removed (CPU only)
  ...
  async installComfyUIManagerRequirements(callbacks?: ProcessCallbacks): Promise<void> {
    // ComfyUI Lite: No ComfyUI-Manager, skip
    log.info('ComfyUI Lite: Skipping ComfyUI-Manager requirements (not included)');
  }
   ...
   */
  // ComfyUI Lite: NVIDIA torch upgrade checks removed (CPU only)
