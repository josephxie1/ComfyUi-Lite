...
export class ComfyInstallation {
  ...
  static async fromConfig(): Promise<ComfyInstallation | undefined> {
    ...
    if (state && basePath) {
      // If the data directory was deleted, reset to fresh install
      if (!existsSync(basePath)) {
        log.info(`Base path ${basePath} no longer exists. Resetting to fresh install.`);
        config.delete('installState');
