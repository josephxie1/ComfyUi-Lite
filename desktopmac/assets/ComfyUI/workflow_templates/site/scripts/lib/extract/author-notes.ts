...
export function extractAllWorkflowText(workflow: WorkflowJson): ExtractedWorkflowText {
  ...
  for (const node of workflow.nodes || []) {
    ...
    if (NOTE_TYPES.has(nodeType) || TEXT_TYPES.has(nodeType)) {
      ...
      examplePrompts.push(...extractStringWidgets(node as Record<string, unknown>));
    }
  ...
  const groups = ((workflow as Record<string, unknown>).groups as { title?: string }[]) || [];
