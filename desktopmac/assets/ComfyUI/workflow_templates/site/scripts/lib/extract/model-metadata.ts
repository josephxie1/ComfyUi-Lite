...
const MODEL_EXTENSIONS = ['.safetensors', '.ckpt', '.pt', '.pth', '.bin', '.onnx', '.gguf'];
...
export function extractWorkflowModels(workflow: WorkflowJson): WorkflowModelRef[] {
  ...
  results.sort((a, b) => a.kind.localeCompare(b.kind) || a.filename.localeCompare(b.filename));
  return results;
}
