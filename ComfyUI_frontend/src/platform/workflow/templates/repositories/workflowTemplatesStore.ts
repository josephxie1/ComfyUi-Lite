...
export const useWorkflowTemplatesStore = defineStore(
  ...
  () => {
    ...
    const enhancedTemplates = computed<EnhancedTemplate[]>(() => {
      ...
      userTemplatesStore.templates.forEach((ut) => {
        const enhancedTemplate: EnhancedTemplate = {
          name: ut.name,
          title: ut.name,
          ...
          mediaType: 'image',
          mediaSubtype: 'webp',
          sourceModule: 'user-custom',
          category: 'Custom',
          categoryType: 'user-custom',
          searchableText: `${ut.name} ${ut.description} custom user`
        }
        allTemplates.push(enhancedTemplate)
      })
    ...
    const filterTemplatesByCategory = (categoryId: string) => {
      ...
      if (categoryId === 'user-custom') {
        return enhancedTemplates.value.filter(
          (t) => t.sourceModule === 'user-custom'
        )
      }
    ...
    const navGroupedTemplates = computed<(NavItemData | NavGroupData)[]>(() => {
      ...
      // 3.5. User Custom Templates
      items.push({
        id: 'user-custom',
        label: st('templateWorkflows.category.Custom', 'Custom'),
        icon: 'icon-[lucide--bookmark]'
      })

      // 3.6. Partner Nodes - virtual category for OpenSource === false templates
      ...
      const extensionCounts = enhancedTemplates.value.filter(
        (t) => t.sourceModule !== 'default' && t.sourceModule !== 'user-custom'
      ).length
        ...
        const extensionModules = Array.from(
          new Set(
            enhancedTemplates.value
              .filter((t) => t.sourceModule !== 'default' && t.sourceModule !== 'user-custom')
    ...
    async function loadWorkflowTemplates() {
      try {
        if (!isLoaded.value) {
          ...
          await useUserTemplatesStore().loadUserTemplates()
          isLoaded.value = true
        }
      } catch (error) {
