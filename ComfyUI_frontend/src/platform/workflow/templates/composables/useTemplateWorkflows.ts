...
export function useTemplateWorkflows() {
  ...
  const getTemplateThumbnailUrl = (
    ...
    if (sourceModule === 'user-custom') {
      const userTemplatesStore = useUserTemplatesStore()
      const userTemplate = userTemplatesStore.templates.find(
        (t) => t.name === template.name
      )
      if (userTemplate?.hasThumbnail) {
        return userTemplatesStore.getThumbnailUrl(template.name)
      }
      return ''
    }
  ...
  const loadWorkflowTemplate = async (id: string, sourceModule: string) => {
    ...
    try {
      // Handle user-custom templates
      if (sourceModule === 'user-custom') {
        const userTemplatesStore = useUserTemplatesStore()
        const json = await userTemplatesStore.getUserTemplateWorkflow(id)
