...
export class LGraphCanvas implements CustomEventDispatcher<LGraphCanvasEventMap> {
  ...
  showLinkMenu(segment: LinkSegment, e: CanvasPointerEvent): boolean {
    ...
    const options = [t('contextMenu.Add Node'), t('contextMenu.Add Reroute'), null, t('contextMenu.Delete'), null]
    ...
    function inner_clicked(
      ...
      switch (v) {
        ...
        case t('contextMenu.Add Reroute'): {
          try {
            this.emitBeforeChange()
            this.adjustMouseEvent(e)
  ...
  showConnectionMenu(
    ...
    const options = [t('contextMenu.Add Node'), t('contextMenu.Add Reroute'), null]

    if (opts.allow_searchbox) {
      options.push(t('contextMenu.Search'), null)
    }
    ...
    const slotTypesDefault = isFrom
      ? LiteGraph.slot_types_default_out
      : LiteGraph.slot_types_default_in
    // Map from translated display name back to original node type ID
    const displayNameToNodeType = new Map<string, string>()
    if (slotTypesDefault?.[fromSlotType]) {
      if (typeof slotTypesDefault[fromSlotType] == 'object') {
        for (const typeX in slotTypesDefault[fromSlotType]) {
          const nodeType = slotTypesDefault[fromSlotType][typeX]
          const displayName = st(
            `nodeDefs.${normalizeI18nKey(nodeType)}.display_name`,
            nodeType
          )
          displayNameToNodeType.set(displayName, nodeType)
          options.push(displayName)
        }
      } else {
        const nodeType = slotTypesDefault[fromSlotType]
        const displayName = st(
          `nodeDefs.${normalizeI18nKey(nodeType)}.display_name`,
          nodeType
        )
    ...
    function inner_clicked(
      ...
      switch (v) {
        ...
        case t('contextMenu.Search'):
          ...
          break
        default: {
          // Resolve translated display name back to original node type ID
          const nodeType = v ? (displayNameToNodeType.get(v) ?? v) : v
          const customProps = {
