...
export class ComfyApi extends EventTarget {
  ...
  /**
   * Deletes one or more physical files from the server via DELETE /view
   * @param { string[] } files The names of the files to delete
   * @param { string } type The type of file (e.g. 'output' or 'input')
   */
  async deleteFiles(files: string[], type: string = 'output') {
    const resp = await this.fetchApi('/view', {
      method: 'DELETE',
      headers: {
        'Content-type': 'application/json'
      },
    ...
    if (resp.status !== 200 && resp.status !== 207) {
      throw new Error(
        `Error deleting files: ${resp.status} ${(await resp).statusText}`
      )
    }
