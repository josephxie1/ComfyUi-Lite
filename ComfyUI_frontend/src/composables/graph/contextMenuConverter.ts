...
export function convertContextMenuToOptions(
  ...
  for (const item of items) {
    ...
    // Try to translate the label, fallback to original content if translation doesn't exist
    const translationKey = `contextMenu.${item.content}`
    const label = t(translationKey) !== translationKey ? t(translationKey) : item.content
