...
async function fetchInputFilesFromAPI(): Promise<AssetItem[]> {
  const response = await fetch(api.internalURL('/files/input?full_info=true'), {
    headers: {
      'Comfy-User': api.user
    }
  })
  ...
  return files.map((file, index) =>
    mapInputFileToAssetItem(file.name, index, 'input', file.size, file.mtime)
  )
}
...
async function fetchOutputFilesFromAPI(): Promise<AssetItem[]> {
  const response = await fetch(api.internalURL('/files/output?full_info=true'), {
    headers: {
      'Comfy-User': api.user
    }
  })
  ...
  const files: Array<{ name: string; size: number; mtime: number }> =
    await response.json()
  return files.map((file, index) =>
    mapInputFileToAssetItem(file.name, index, 'output', file.size, file.mtime)
  )
}
...
export const useAssetsStore = defineStore('assets', () => {
  ...
  const fetchHistoryAssets = async (loadMore = false): Promise<AssetItem[]> => {
    ...
    // In OSS mode, when history is exhausted (or empty after server restart),
    // fetch raw physical files to ensure previously generated outputs remain visible.
    let rawFiles: AssetItem[] = []
    if (!isCloud && history.length < BATCH_SIZE) {
      try {
        rawFiles = await fetchOutputFilesFromAPI()
        hasMoreHistory.value = false // We brought in all raw files at once, so pagination is complete
      } catch (err) {
        console.error('Failed to fetch raw output files:', err)
    ...
    // Filter out raw files that we already loaded via history
    const existingNames = new Set(allHistoryItems.value.map(a => a.name))
    newAssets.forEach(a => existingNames.add(a.name))
    
    const missingRawFiles = rawFiles.filter(a => !existingNames.has(a.name))
    newAssets.push(...missingRawFiles)
  ...
  /**
   * Delete specific input assets by filename
   */
  const deleteInputAssets = async (files: string[]) => {
    try {
      const response = await api.deleteFiles(files, 'input')
      const deletedFiles = response.deleted || []
      
      if (deletedFiles.length > 0) {
        inputAssets.value = inputAssets.value.filter(
          (asset) => !deletedFiles.includes(asset.name)
        )
      }
      return deletedFiles.length > 0
    } catch (err) {
      console.error('Error deleting input assets:', err)
      return false
    }
  }
  ...
  /**
   * Delete specific history assets by ID/filename
   */
  const deleteHistoryAssets = async (files: string[]) => {
    try {
      const response = await api.deleteFiles(files, 'output')
      const deletedFiles: string[] = response.deleted || []
      
      if (deletedFiles.length > 0) {
        // Collect IDs before filtering them out of the array
        const historyIds = allHistoryItems.value
          .filter((asset) => deletedFiles.includes(asset.name))
          .map((asset) => asset.id)
        ...
        historyAssets.value = allHistoryItems.value
        deletedFiles.forEach((f: string) => loadedIds.delete(f))

        // Also notify backend to clear history metadata
        for (const id of historyIds) {
          try {
            await api.deleteItem('history', id)
          } catch (e) {
            console.error('Failed to sweep history metadata:', e)
          }
        }
      }
      ...
      console.error('Error deleting history assets:', err)
      return false
    }
  }
