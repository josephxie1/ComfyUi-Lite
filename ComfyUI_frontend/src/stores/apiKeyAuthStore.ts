...
export const useApiKeyAuthStore = defineStore('apiKeyAuth', () => {
  ...
  const initializeUserFromApiKey = async () => {
    ...
    if (!createCustomerResponse) {
      // Don't clear the API key here - it may still be valid for prompt execution
      // even if createCustomer fails (e.g. network issues through dev proxy)
      console.warn('createCustomer failed, but keeping API key for prompt use')
      return
    }
