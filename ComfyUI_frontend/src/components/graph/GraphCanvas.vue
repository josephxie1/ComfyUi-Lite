...
onMounted(async () => {
  ...
  // Check if we're loading after a version revert
  const revertData = sessionStorage.getItem('workflow_revert_data')
  if (revertData) {
    sessionStorage.removeItem('workflow_revert_data')
    try {
      const workflowData = JSON.parse(revertData)
      await comfyApp.loadGraphData(workflowData, true, true, null, {
        showMissingNodesDialog: false,
        showMissingModelsDialog: false
      })
    } catch (e) {
      console.error('Failed to load reverted workflow:', e)
      // Fall back to normal workflow persistence
